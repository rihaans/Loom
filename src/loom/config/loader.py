...
USER_CONFIG_DIR = Path.home() / ".loom"
...
def find_config_file() -> Path | None:
    ...
    Search order:
    1. Current directory: ./loom.toml
    2. User config: ~/.loom/config.toml
    3. None if not found
...
def load_env_config() -> dict[str, Any]:
    ...
    - LOOM_LLM_DEFAULT: default LLM (provider:model)
    - LOOM_LLM_<ROLE>: per-agent LLM override
    - LOOM_OUTPUT_DIR: output directory
    - LOOM_MAX_RETRIES: max QA retries
    - LOOM_USE_DOCKER_SANDBOX: true/false
    - LOOM_SANDBOX_TIMEOUT: timeout in seconds
    - LOOM_SANDBOX_MEMORY_MB: memory limit in MB
    ...
    # Default LLM
    if llm_default := os.getenv("LOOM_LLM_DEFAULT"):
    ...
    for env_suffix, role in role_env_map.items():
        if llm_str := os.getenv(f"LOOM_LLM_{env_suffix}"):
    ...
    # Output directory
    if output_dir := os.getenv("LOOM_OUTPUT_DIR"):
        config["output_dir"] = output_dir

    # Max retries
    if max_retries := os.getenv("LOOM_MAX_RETRIES"):
        config["max_retries"] = int(max_retries)

    # Sandbox settings
    if use_docker := os.getenv("LOOM_USE_DOCKER_SANDBOX"):
        config["use_docker_sandbox"] = use_docker.lower() in ("true", "1", "yes")

    if timeout := os.getenv("LOOM_SANDBOX_TIMEOUT"):
        config["sandbox_timeout_seconds"] = int(timeout)

    if memory := os.getenv("LOOM_SANDBOX_MEMORY_MB"):
        config["sandbox_memory_mb"] = int(memory)

    # Cost budget
    if budget := os.getenv("LOOM_COST_BUDGET_USD"):
...
def save_config(config: LoomConfig, path: Path | None = None) -> Path:
    ...
    Args:
        ...
        path: Path to save to (defaults to ./loom.toml)
