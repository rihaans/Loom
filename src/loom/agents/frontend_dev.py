...
async def frontend_dev_node(
    ...
    budget_error = check_budget(state, config, "frontend developer")
    if budget_error:
    ...
    for attempt in range(1, MAX_PARSE_RETRIES + 1):
        try:
            ...
            last_error = e
            logger.warning(f"Frontend dev attempt {attempt}/{MAX_PARSE_RETRIES} failed: {e}")
    ...
    error_message = (
        f"Failed to generate frontend code after {MAX_PARSE_RETRIES} attempts: {last_error}"
    )
